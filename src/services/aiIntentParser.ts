...
Indica cosa desideri prenotare, per quante persone e l'orario desiderato.`,
  ...
  const proposal: ParsedIntent = {
    ...
    explanation: `Ho predisposto la tua prenotazione per **${venueName}** (${venueLocation}) per **${dateText.toLowerCase()}** alle **${timeSlot}** per **${count} persona/e**.\n\nDeposito cauzionale: **${DEPOSIT_AVAX} AVAX** (rimborsato al check-in). Clicca per confermare su Avalanche.`,
    isOutdoor,
    groupId,
  };
